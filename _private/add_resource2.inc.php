	...
	if ($watcherRole<1) header('Location: index.php?page=direwolf&userid=' . $currentUser);
		...
		if ($bodyId == -1) {
			...
			if ($watcherRole>1) {
				...
				if (!isset($_GET["source"])||!isset($_GET["pid"])||!isset($_GET["slot"])||!isset($_GET["res"])) {
					header('Location: index.php?page=viewchar&charid=' . $charcheck . '&userid=' . $currentUser . '&tab=8');
				}
				else {
					if (!is_numeric($_GET["source"])||!is_numeric($_GET["pid"])||!is_numeric($_GET["slot"])||!is_numeric($_GET["res"])) {
						include_once "header2.inc.php";
						...
						$info = $entry->addResFromSource($_GET["source"], $_GET["slot"], $_GET["res"]);
						...
						else if ($info==-5) para("Error: Invalid pile selection.");
						else if ($info==-6) para("Error: Adding resource failed.");
