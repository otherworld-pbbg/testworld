				...
				if (!isset($_GET["pid"])||!isset($_GET["slot"])) {
					...
					if (!is_numeric($_GET["pid"])||!is_numeric($_GET["slot"])) {
						...
						if ($entry->invested>0) {
							...
							ptag("h1", "Attempting to remove resource from the project");
							para("This project already has progress so you can't take stuff off anymore.");
						}
						else {
							$success = $entry->emptyResourceSlot($_GET["slot"]);
							if ($success==-1) {
								include_once "header2.inc.php";
								echo "<div class='displayarea'>";
								ptag("h1", "Attempting to remove resource from the project");
								...
								para("Removing the resource failed for some reason.");
								include_once "header2.inc.php";
								echo "<div class='displayarea'>";
								ptag("h1", "Attempting to remove resource from the project");
							}
								...
								echo "<div class='displayarea'>";
								ptag("h1", "Attempting to remove resource from the project");
								para("The slot was emptied but accidentally the resources inside got deleted. Sorry about that.");
							}
