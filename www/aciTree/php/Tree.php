...
abstract class Tree {
    ...
    private function _json($parentId, Array &$json, $children) {
        $branch = $this->branch($parentId);//The result is an array but the function needs to be redefined elsewhere
        foreach ($branch as $id => $label) {
            $props = $this->itemProps($id);
            $list = array();
            if ($children) {
                $this->_json($id, $list, $children);//if children is true, it recursively gets everything
                if (count($list) == 0) {
                    $props['inode'] = false;
                }
            }
    ...
    public function json($parentId, $children = false) {
        $json = array();
        $this->_json($parentId, $json, $children);//children is either true or false
